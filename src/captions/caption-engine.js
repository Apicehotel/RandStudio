...
export function splitForSocial(segment,styleId='social'){
  const style=CAPTION_STYLES[styleId]||CAPTION_STYLES.social,words=segment.words?.length?segment.words:estimateWords(segment);
  const chunks=[];for(let i=0;i<words.length;i+=style.maxWords){const part=words.slice(i,i+style.maxWords);chunks.push({id:`${segment.id}-chunk-${i}`,start:part[0]?.start??segment.start,end:part.at(-1)?.end??segment.end,text:part.map(w=>w.text).join(' '),words:part});}
...
export function captionGraphic(segment,styleId='social',time=segment.start){
  ...
  return{schema:'randstudio.graphic/v1',type:'caption',text,fontSize:style.fontSize,fontWeight:style.fontWeight,color:style.color,highlight:style.highlight,background:style.background,position:style.position,activeWord:word?.text||'',padding:22,align:'center'};
}
