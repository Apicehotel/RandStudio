import { createProject } from './composition.js';
...
export function applyProjectTemplate(project,id){
  const t=PROJECT_TEMPLATES[id];
  if(!t)throw new Error(`Template sconosciuto: ${id}`);
  const base=isValidProject(project)
    ? structuredClone(project)
    : createProject({name:'Nuovo progetto',width:t.width,height:t.height,fps:t.fps});
  return{...base,width:t.width,height:t.height,fps:t.fps,templateId:id,updatedAt:new Date().toISOString()};
}

function isValidProject(project){
  return !!(project&&typeof project==='object'&&project.id&&Array.isArray(project.tracks));
}
