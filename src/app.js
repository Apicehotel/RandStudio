...
import { STICKER_PACK, createGraphicSpec, isGraphicClip, prepareGraphicMedia, renderGraphicBlob } from './graphics/overlay-engine.js';
import { applyMotionPreset, evaluateMotion } from './graphics/motion-engine.js';
import { CAPTION_STYLES, normalizeTranscript, parseSRT, parseVTT, toSRT, toVTT, splitForSocial, captionGraphic } from './captions/caption-engine.js';
import { createTranscriptionRegistry } from './captions/transcription-provider.js';
...
ui.whisperEndpoint.value = localStorage.getItem('randstudio-whisper-endpoint') || ui.whisperEndpoint.value;
ui.whisperEndpoint.addEventListener('change', () => localStorage.setItem('randstudio-whisper-endpoint', ui.whisperEndpoint.value.trim()));

$('#addTitleBtn').onclick = () => addGraphic('text', createGraphicSpec('text', { text: 'Titolo' }), 'Titolo');
$('#addLowerThirdBtn').onclick = () => addGraphic('lower-third', createGraphicSpec('lower-third'), 'Lower third');
$('#addCalloutBtn').onclick = () => addGraphic('callout', createGraphicSpec('callout'), 'Callout');
$('#addArrowBtn').onclick = () => addGraphic('arrow', createGraphicSpec('arrow'), 'Freccia');
$('#addLightLeakBtn').onclick = () => addGraphic('light-leak', createGraphicSpec('light-leak'), 'Light leak', 'fade');
$('#freezeFrameBtn').onclick = freezeFrame;
...
for (const sticker of STICKER_PACK) {
  const b = document.createElement('button'); b.textContent = sticker; b.title = `Sticker ${sticker}`;
  b.onclick = () => addGraphic('sticker', createGraphicSpec('sticker', { text: sticker }), `Sticker ${sticker}`, 'pop'); $('#stickerStrip').append(b);
}
...
async function importFiles(files) {
  for (const file of files) {
    ...
    if (type === 'lut') { try { item.lut = parseCubeLUT(await file.text(), file.name); } catch (error) { status(`LUT non valida: ${error.message}`); continue; } }
...
async function addGraphic(type, graphic, name, motionPreset = 'none') {
  ...
  clip = applyMotionPreset(clip, motionPreset); const next = addClip(p, t.id, clip); store.commit(next); selected = { trackId: t.id, clipId: id };
  await ensureGraphicMedia(findClip(store.value, t.id, id).clip); persistProject(); renderMedia(); render();
}

async function ensureGraphicMedia(clip) {
  if (!isGraphicClip(clip)) return null;
  const id = `graphic:${clip.id}`, old = media.get(id); if (old?.url) URL.revokeObjectURL(old.url);
  const blob = await renderGraphicBlob(clip.graphic), item = { id, file: blob, type: 'image', name: `${clip.name || clip.type}.png`, duration: clip.end - clip.start, virtualName: `graphic-${clip.id}.png`, url: URL.createObjectURL(blob), generated: true };
...
function render() {
  const p = store.value; ui.undo.disabled = !store.canUndo; ui.redo.disabled = !store.canRedo;
  const duration = Math.max(1, projectDuration(p)); ui.duration.textContent = `${duration.toFixed(1)} s`; ui.playhead.max = String(duration); ui.ruler.style.width = `${Math.max(800, duration * pxPerSecond)}px`; ui.tracks.innerHTML = '';
  for (const track of p.tracks) {
    const row = document.createElement('div'); row.className = `track-row ${track.kind === 'audio' ? 'audio' : ''}`; const label = document.createElement('div'); label.className = 'track-label'; label.textContent = track.name; const lane = document.createElement('div'); lane.className = 'track-lane'; lane.style.width = `${Math.max(800, duration * pxPerSecond)}px`;
    for (const clip of track.clips) lane.appendChild(renderClip(track, clip)); row.append(label, lane); ui.tracks.append(row);
  }
  renderInspector(); syncPreviewToPlayhead(); updateTransport();
}

function renderClip(track, clip) {
  const el = document.createElement('div'); el.className = `clip ${isGraphicClip(clip) ? 'graphic-clip' : ''} ${clip.type === 'caption' ? 'caption-clip' : ''}`;
  if (selected?.clipId === clip.id) el.classList.add('selected'); if (!media.has(clip.sourceId) && !isGraphicClip(clip)) el.classList.add('missing');
  el.style.left = `${clip.start * pxPerSecond}px`; el.style.width = `${Math.max(28, (clip.end - clip.start) * pxPerSecond)}px`;
  const fx = clip.effects?.length ? ` · ${clip.effects.length} FX` : '', ramp = clip.speedRamp ? ` · RAMP:${clip.speedRamp.preset}` : '';
  el.innerHTML = `<strong>${escapeHtml(clip.name)}</strong><span>${clip.start.toFixed(1)} → ${clip.end.toFixed(1)}${fx}${clip.lutSourceId ? ' · LUT' : ''}${clip.motion?.preset && clip.motion.preset !== 'none' ? ` · ${clip.motion.preset}` : ''}${clip.playbackRate && clip.playbackRate !== 1 ? ` · ${clip.playbackRate}×` : ''}${ramp}${clip.type === 'caption' ? ' · CAP' : isGraphicClip(clip) ? ' · GFX' : ''}</span>`;
  ...
  el.onpointerdown = (e) => beginDrag(e, el, track.id, clip.id, clip.start); return el;
}

function beginDrag(event, el, trackId, clipId, start) {
  if (event.button !== 0) return; const origin = event.clientX; el.setPointerCapture(event.pointerId);
...
function renderMedia() {
  ...
  for (const item of media.values()) {
    if (item.generated) continue; const div = document.createElement('div'); div.className = 'media-item'; const icon = item.type === 'video' ? '▶' : item.type === 'audio' ? '♫' : item.type === 'lut' ? '◫' : '▧';
...
function renderInspector() {
  const f = selected && findClip(store.value, selected.trackId, selected.clipId); ui.inspector.hidden = !f; ui.inspectorEmpty.hidden = !!f; ui.graphicInspector.hidden = !f || !isGraphicClip(f.clip) || f.clip.type === 'light-leak';
  if (!f) { if (ui.speedRampStatus) ui.speedRampStatus.textContent = 'Nessuna curva velocità.'; return; }
  ...
  if (ui.speedRampStatus) ui.speedRampStatus.textContent = c.speedRamp ? `Curva: ${c.speedRamp.preset} · ${c.speedRamp.samples} segmenti render` : 'Nessuna curva velocità.';
  if (c.speedRamp?.points?.length >= 2) { ui.speedRampStart.value = c.speedRamp.points[0].rate; ui.speedRampMid.value = rateAt(c.speedRamp, .5).toFixed(2); ui.speedRampEnd.value = c.speedRamp.points.at(-1).rate; }
...
async function transcribeSelectedClip() {
  const f = selected && findClip(store.value, selected.trackId, selected.clipId); if (!f || !['video', 'audio'].includes(f.clip.type)) return captionStatus('Seleziona prima una clip audio o video.'); const item = media.get(f.clip.sourceId); if (!item?.file) return captionStatus('Media originale non disponibile.'); captionStatus('Trascrizione in corso…'); const gatewayEndpoint = ui.gateway.value.trim(), whisperEndpoint = ui.whisperEndpoint.value.trim() || 'http://127.0.0.1:8080';
  try { const registry = createTranscriptionRegistry({ gatewayEndpoint, whisperEndpoint }); const providerId = gatewayEndpoint ? 'gateway-stt' : 'whispercpp'; const transcript = await registry.transcribe(providerId, { file: item.file, language: ui.captionLanguage.value.trim() || 'auto', wordTimestamps: true }); const mapped = mapTranscriptToClip(transcript, f.clip); await applyTranscriptToTimeline(mapped, ui.captionStyle.value); captionStatus(`${mapped.segments.length} segmenti creati con ${providerId === 'whispercpp' ? 'Whisper.cpp locale' : 'AI Gateway STT'}.`); } catch (error) { captionStatus(`Trascrizione non riuscita: ${error.message}`); }
}
...
function mapTranscriptToClip(transcript, clip) {
  const t = normalizeTranscript(transcript), sourceIn = Math.max(0, Number(clip.in) || 0), sourceOut = Math.max(sourceIn, Number(clip.out) || sourceIn), segments = [];
  for (const s of t.segments) { const a = Math.max(sourceIn, s.start), b = Math.min(sourceOut, s.end); if (b <= a) continue; const words = (s.words || []).filter((w) => w.end > sourceIn && w.start < sourceOut).map((w) => ({ ...w, start: sourceMomentToTimeline(clip, Math.max(sourceIn, w.start)), end: sourceMomentToTimeline(clip, Math.min(sourceOut, w.end)) })); segments.push({ ...s, start: sourceMomentToTimeline(clip, a), end: sourceMomentToTimeline(clip, b), words }); }
  return { ...t, segments };
}

async function applyTranscriptToTimeline(transcript, styleId = 'social') {
  const normalized = normalizeTranscript(transcript); if (!normalized.segments.length) throw new Error('Trascrizione vuota'); let p = structuredClone(store.value); p.tracks = p.tracks.map((t) => ({ ...t, clips: t.clips.filter((c) => c.type !== 'caption') })); let track = p.tracks.find((t) => t.id === 'captions-1'); if (!track) { track = { id: 'captions-1', name: 'Sottotitoli', kind: 'visual', clips: [] }; p.tracks.push(track); } const style = CAPTION_STYLES[styleId] || CAPTION_STYLES.social, pending = [];
  for (const segment of normalized.segments) { const chunks = splitForSocial(segment, styleId); for (const chunk of chunks) { if (styleId === 'karaoke' && chunk.words?.length) { for (const word of chunk.words) pending.push({ segment: chunk, start: word.start, end: Math.max(word.start + .05, word.end), activeWord: word.text }); } else pending.push({ segment: chunk, start: chunk.start, end: chunk.end, activeWord: '' }); } }
  for (const row of pending) { const id = crypto.randomUUID(), graphic = { ...captionGraphic(row.segment, styleId, row.start), activeWord: row.activeWord }, duration = Math.max(.05, row.end - row.start); p = addClip(p, 'captions-1', { id, sourceId: `graphic:${id}`, sourceName: 'caption.png', name: row.segment.text, type: 'caption', start: row.start, in: 0, out: duration, duration, graphic, caption: { styleId: style.id, segmentId: row.segment.id, activeWord: row.activeWord } }); }
  p.captionTranscript = normalized; p.captionStyle = styleId; store.commit(p); await hydrateGraphics(); await persistProject(); renderMedia(); render();
}
...
async function showMedia(item, clip = null, t = 0) {
  ...
  else if (item.type === 'video') { if (ui.video.src !== item.url) ui.video.src = item.url; ui.video.playbackRate = clipInstantRate(clip, t); ui.video.hidden = false; applyVisualState(ui.video, clip, t); if (clip && Number.isFinite(ui.video.duration)) ui.video.currentTime = Math.min(clip.out, clipSourceTime(clip, t)); if (!playing) await showVisual(ui.video, clip, t); }
  else { if (ui.audio.src !== item.url) ui.audio.src = item.url; ui.audio.playbackRate = clipInstantRate(clip, t); if (clip && Number.isFinite(ui.audio.duration)) ui.audio.currentTime = Math.min(clip.out, clipSourceTime(clip, t)); ui.audio.hidden = false; }
}
...
async function openProject(file) { if (!file) return; try { const data = JSON.parse(await file.text()), errors = validateProject(data); if (errors.length) throw new Error(errors.join('; ')); store = new HistoryStore(data); selected = null; currentTime = 0; await hydrateGraphics(); await persistProject(); renderMedia(); render(); if (store.value.captionTranscript?.segments?.length) captionStatus(`${store.value.captionTranscript.segments.length} segmenti sottotitoli caricati.`); } catch (error) { alert(`Progetto non valido: ${error.message}`); } }
async function exportVideo() { try { status('Preparazione overlay e export…'); ui.progress.value = 0; const prepared = await prepareGraphicMedia(store.value, media), compiled = compileProject(prepared.project, prepared.media); status('Caricamento FFmpeg / rendering locale…'); const blob = await renderWithFFmpeg(compiled, (p) => ui.progress.value = p); download(blob, compiled.outputName); status('Export completato'); ui.progress.value = 1; } catch (error) { console.error(error); status(`Export non riuscito: ${error.message}`); } }
...
function mediaType(f) { if (extension(f.name) === 'cube') return 'lut'; if (f.type.startsWith('video/')) return 'video'; if (f.type.startsWith('audio/')) return 'audio'; if (f.type.startsWith('image/') || extension(f.name) === 'svg') return 'image'; return null; }
...
function waitVideo(v) { if (v.readyState >= 2) return Promise.resolve(); return new Promise((resolve, reject) => { v.addEventListener('loadeddata', resolve, { once: true }); v.addEventListener('error', () => reject(new Error('Video non leggibile')), { once: true }); }); }
function extension(n) { return n.includes('.') ? n.split('.').pop().toLowerCase() : ''; }
function defaultExt(t) { return t === 'audio' ? 'mp3' : t === 'image' ? 'png' : t === 'lut' ? 'cube' : 'mp4'; }
...
function colorHex(v) { return /^#[0-9a-f]{6}$/i.test(v || '') ? v : '#ffffff'; }
